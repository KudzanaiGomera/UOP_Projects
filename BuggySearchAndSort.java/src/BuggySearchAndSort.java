...
public class BuggySearchAndSort {
	...
	public static boolean contains(int[] array, int val) {
		for (int i = 0; i < array.length; i++) {
			if (array[i] == val)
				return true;
			/*else
				return false;*/ /// deadcode it wont get executed
		}
		return false;
	}
	...
	public static void bubbleSort(int[] array) {
		for (int i = 0; i < array.length; i++) {
			for (int j = 0; j < array.length-1; j++) { /// incrementing i instead of j
	...
	public static void selectionSort(int[] array) {
		for (int top = array.length - 1; top > 0; top--) {
			int positionOfMax = 0;
			for (int i = 1; i <= top; i++) {
				if (array[i] > array[positionOfMax])/// indexing with 1 instead of using i
					positionOfMax = i;
			}
	...
	public static void insertionSort(int[] array) {
		for (int top = 1; top < array.length; top++) {
			...
			int pos = top;
			while (pos > 0 && array[pos-1] > temp) {/// changed the position up -1
